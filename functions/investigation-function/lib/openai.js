...
async function generateCaseSummary(caseData, app) {
  const messages = [
    { role: 'system', content: 'You are an expert in law enforcement case summarization. Write concise, factual case summaries for Karnataka Police investigators.' },
    { role: 'user', content: `Generate a structured case summary for this FIR:\n${JSON.stringify(caseData)}` }
  ];
  return callOpenAI(messages, 0.4, 1000, app);
}

async function generateInvestigationLeads(caseData, similarCases, app) {
  const messages = [
    { role: 'system', content: 'You are an expert investigative advisor for Karnataka Police. Analyze case gaps and suggest actionable investigative leads.' },
    { role: 'user', content: `Suggest 3-5 investigative leads for this case:\n\nCase: ${JSON.stringify(caseData)}\n\nSimilar past cases: ${JSON.stringify(similarCases.slice(0, 3))}` }
  ];
  return callOpenAI(messages, 0.6, 1000, app);
}

module.exports = { generateCaseSummary, generateInvestigationLeads, callOpenAI };
